...
Furthermore, frequency of 
anxious symptoms (`r lit_plots$depr_anx_stats %>% filter(disorder == 'anxiety', source == 'cohort') %>% mutate(text = paste0(signif(percent, 2), '% [95% CI: ', signif(lower_ci, 2), ' to ', signif(upper_ci, 2), '%]')) %>% .$text`) assessed by the GAD-2 instrument was significantly higher than in the general German population within the time frame of the study survey 
(`r lit_plots$depr_anx_stats %>% filter(disorder == 'anxiety', source == 'desurv') %>% mutate(text = paste0(signif(percent, 2), '% [95% CI: ', signif(lower_ci, 2), ' to ', signif(upper_ci, 2), '%]')) %>% .$text`). 
Depressive symptoms measured by the PHQ-2 tool were, in turn, much less prevalent 
in our cohort (`r lit_plots$depr_anx_stats %>% filter(disorder == 'depression', source == 'cohort') %>% mutate(text = paste0(signif(percent, 2), '% [95% CI: ', signif(lower_ci, 2), ' to ', signif(upper_ci, 2), '%]')) %>% .$text`) than 
estimates for the general German population (`r lit_plots$depr_anx_stats %>% filter(disorder == 'depression', source == 'desurv') %>% mutate(text = paste0(signif(percent, 2), '% [95% CI: ', signif(lower_ci, 2), ' to ', signif(upper_ci, 2), '%]')) %>% .$text`). 
Finally, frequency of moderate-to-severe symptoms of depression (PHQ-8 $\geq$ 10) was comparable in the study samples and the Austrian microcensus results (__Supplementary Figure S5AB__). 
These results let us reason that, despite the mountain accident, the overall mental health status of the study cohort was comparable to the general population of German-speaking countries and we elaborate on this in __Results/Characteristic of the study cohort__ and __Discussion__.
...
We corrected the ambiguous passages and describe frequency of manifest PTSD solely in __Results/Characteristic of the study cohort__. 
We use the term 'symptoms of PTSD elsewhere in the revised text.
...
From comparisons of our sample with the general Austrian population (__Supplementary Table S4__) and the collective of mountain accident victims in Austria in 2023 (__Supplementary Figure S1__), we infer that the study cohort consisted of significantly more males, highly educated and healthy individuals and victims of winter sport accidents as expected for the general population. 
Effect size of those differences was weak (Cramer's V effect size statistic $\leq$ 0.1)
...
Anxious symptoms measured with the GAD-2 instrument were more common in the study cohort than in the German population during the survey [@Junker2023]. 
In turn, depending on the tool and reference population, overall depressive symptoms and symptoms of moderate-to-severe depression were lower or comparable with the general German or Austrian population (__Supplementary Figure S5__). 
We discuss in __Results/Characteristic of the study cohort__ and in __Discussion__, that the overall frequency of mental disorder symptoms in our cohort is, in spite of the accident, comparable with the general population. 
...
To support this notion with hard figures, we compared average scores with samples from the general German [@Leppert2008] and Tyrolean/West Austrian population [@Chernova2021], and with resilience scores published by Mikutta and colleagues for a cohort of Swiss mountain rescue service workers [@Mikutta2022]. 
...
This again, may at least in part explain low frequency of manifest PTSD and PTSD symptoms in those two collectives (__Supplementary Figure S4B__). 
...
The dark side of popularity of mountain sports are accidents whose number exceeded `r table(lit_kurasi$data$sport_type$subset)['Austria'] %>% signif(2)` in 2023 in the nation-wide scale, according to the official counts by the safety board and police [@OsterreichischesKuratoriumfurAlpineSicherheit2024]. 
By comparison, this number corresponds approximately to the yearly sum incidence of prostate and breast cancer in Austria [@StatistikAustria2023a]. 
...
We can hence only speculate, that the excluded survey participants found questions of the questionnaire concerning their fears, concerns and life satisfaction too private. 
...
Additionally, we did not ask about severity or time frame of mountain sport accidents in the past, so it is hard to judge if an how they affected mental health at the time point of the survey. 
...
For these reasons it is unlikely that mountain accidents in the past had an substantial effect on the outcome of our study. 
