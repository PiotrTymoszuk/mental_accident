...
By this means, three mental clusters were defined: the neutral, PTG (post-traumatic growth) and PTSD (post-traumatic stress disorder) clusters (__Supplementary Figure S\@ref(fig:fig-clust-dev)B__). 
...
flextable::flextable(suppl_tables$partition) %>% 
  ...
  flextable::width(5, width = 2.3, unit = 'cm') %>% 
  footnote(1, 4:5, 
           value = as_paragraph('\u03C7\u00B2 test with Cramer V effect size statistic'), 
           part = 'body', 
           ref_symbols = 'a') %>% 
  footnote(2, 1, 
...
```{r fig-early-crf, fig.width = mm_inch(suppl_figures$early_crf$w), fig.height = mm_inch(suppl_figures$early_crf$h), fig.cap = 'Assignment of accident victims to the mental clusters based on explanatory factors available during acute medical management of the accident.'}

suppl_figures$early_crf$plot

```

__Supplementary Figure S\@ref(fig:fig-early-crf). Assignment of accident victims to the mental clusters based on explanatory factors available during acute medical management of the accident.__ 

_A conditional random forest classifier for the mental cluster assignment based on demographic, socioeconomic, clinical and accident-related factors available during early acute medical management of the accident was trained in the training subset of the study cohort._
